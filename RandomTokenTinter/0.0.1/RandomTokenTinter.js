/***************************************************************************************************
    ...
    '--tinttable -TABLENAME' indicates that the list of tints is contained in the rollable table TABLENAME,
                             ...
                             or the word 'transparent' to indicate the removal of tinting.  Name entries can be
                             followed by a space-delimited comment (i.e. #FF0000 Bright Red)).  --tinttable will
    ...
    Version 0.0.1 21-April-2022: Minor update to add the ability to append comments to tinttable, and
                  to add a seventh "cyan" color to the default tint template.                  
...
*/            
        ...
        if (args.includes('--help')) { // Check for help request.
            ...
            var helpText = '<b>Random Token Tinter</b> <i>v.' + version + '</i><br><i>Created by Bill (Do Not Kill)</i><br>' +
                       ...
                       '--pc will cause selected (or ID-listed) player character tokens to be affected as well (by default they are ignored).<br>' +
                       ...
                       'It is acceptable to have a single entry in --tintlist to tint all targeted tokens the same color (or to remove '
                       'tinting from said tokens).  ' +
                       ...
                       '"transparent" to indicate the removal of tinting.  "Name" entries can be followed by a space-delimited '
                       'comment - i.e. #FF0000 Bright Red.  --tinttable will be ignored if a valid --tintlist ' +
...
/*
   Declare the default tint list in one place for maintenance purposes
        ...
        var defaultTintList = [ '#cc0000', '#ee6600', '#ffff00', '#339020', '#00ffff', '#0000cc', '#6600cc' ];
/*
...
/*
   The table entry name I need is really deeply buried in the roll structure, and then potentially needs to
   be parsed by spaces to eliminate a potential trailing comment.
*/
                        var ttNameStr = roll.rolls[0].results[0].tableItem.name.split(' ');
                        tintVal = ttNameStr[0]; // The tint value will be the first (or possibly only) item in the list
